...
from ctxwatch.models.async_migration import AsyncMigration, MigrationStatus
...
def execute_op(sql: str, args=None, *, query_id: str, timeout_seconds: int = 600, settings=None):
    from ctxwatch.clickhouse.client import run_query
...
def process_error(
    ...
    from ctxwatch.async_migrations.runner import attempt_migration_rollback

    attempt_migration_rollback(migration)


def trigger_migration(migration: AsyncMigration, fresh_start: bool = True):
    from ctxwatch.tasks import run_async_migration
...
#     2. Our Celery tasks are not essential for the functioning of KhulnaSoft, meaning losing a task is not the end of the world
#     """
...
#     from khulnasoft.async_migrations.runner import attempt_migration_rollback
